...
while($operation){
    ...
    elseif($choice -eq 3){ 
        ...
        #              - If the given string satisfy the conditions, returns tru
        ...
        $passwordAllowed = checkPassword $password
        if(!$passwordAllowed){
            Write-Host "Password does not meet requirements." | Out-String
            continue #skip rest of function and go back to menu
        }
    ...
    #       or a character that should not be accepted. Give a proper message to the user and prompt again.
        ...
        Write-Host "Invalid selection: '$choice'. Please enter a number between 1 and 10." | Out-String
    }
}
